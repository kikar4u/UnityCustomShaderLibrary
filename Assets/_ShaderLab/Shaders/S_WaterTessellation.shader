...
Shader "Sylve/S_WaterTessellation"
{
    ...
    Properties
    {
        ...
        _CellSize("Cell Size", Range(0, 10)) = 0.5
...
#include "hlsl/CustomTessellation.hlsl"
#include "lib/Random.cginc"
#include "lib/NoiseLib.cginc"
...
#pragma domain domain
    ...
    {
        Varyings output;
        float2 value = _CellSize * input.uv;
        float Noise = voronoiNoise(value) * _Strength;
    ...
    {
        float2 value = _CellSize * IN.uv;
        float noise = voronoiNoise(value);
        
        // Use your calculated noise value to modify the final color
        half4 tex = float4(noise, noise, noise, 1.0); // Create a grayscale color using the noise value
